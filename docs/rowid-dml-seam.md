...
| V6 | **The transient locator and the stable id are NOT confused in code, only in names.** The buffered-UPDATE loop uses `rid` for the DV target (`rid >> RowIdPositionBits` → `DeletedByOrdinal`) and `srcTracking[..].Ids` for identity, in adjacent lines. | `dotnet/Fabricator.Delta/DeltaCatalog.cs` ~2745-2765 |
