...
|---|---|---|---|
| 1 | `BackendRegistry.Discover` ([BackendRegistry.cs:112-139](../dotnet/Fabricator.Bridge/ProviderRegistry.cs)) | `Assembly.Load(name)` + `GetTypes()` scan for `IBackend` + `Activator.CreateInstance` | **source-generated registry** (§5) |
| 2 | `ScanPluginDirectories` + `BackendRegistry.Invalidate` ([BackendRegistry.cs](../dotnet/Fabricator.Bridge/ProviderRegistry.cs)) | `AssemblyLoadContext.LoadFromAssemblyPath` + `Resolving` hook; since 2026-08-18 the scan can also RE-run, driven by `fabricator_install_plugin()` | **compile-time plugins** (§6); no runtime equivalent exists under AOT — and the installer has none either, so `fabricator_install_plugin` must be absent from the AOT SKU rather than merely inert (a function that reports success and loads nothing is the silence the scan report exists to remove) |
...
- ~~**R2 — Fluid/Parlot** expression compilation; force interpreted or drop
  `fluid_render` from the SKU.~~ **RETIRED 2026-09-01**: `fluid_render` left the core for
