...
| 2 | `ScanPluginDirectories` + `BackendRegistry.Invalidate` ([BackendRegistry.cs](../dotnet/Fabricator.Bridge/BackendRegistry.cs)) | `AssemblyLoadContext.LoadFromAssemblyPath` + `Resolving` hook; since 2026-08-18 the scan can also RE-run, driven by `fabricator_install_plugin()` | **compile-time plugins** (§6); no runtime equivalent exists under AOT — and the installer has none either, so `fabricator_install_plugin` must be absent from the AOT SKU rather than merely inert (a function that reports success and loads nothing is the silence the scan report exists to remove) |
