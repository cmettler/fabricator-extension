...
| 1.21 | **`COMMENT ON` is WRITE-ONLY — no comment is ever read BACK.** `duckdb_tables().comment` and `duckdb_columns().comment` stay `NULL` for a table whose comment is demonstrably set on the far side, so a comment cannot be round-tripped through DuckDB; it is for annotating a table OTHER tools read. Also **refused on Fabric Warehouse / the Fabric Lakehouse SQL endpoint / Synapse dedicated** (no extended properties), on a provider-declared VIEW, and on the DAX provider | Built 2026-09-08, write-only by decision rather than by accident — the read-back is a different mechanism (comments would have to ride the table-schema crossing as Arrow field metadata, exactly like the column-default read-back gap in the ADD COLUMN note) and a separate slice. Pinned as a CHARACTERIZATION so the day it is implemented arrives as a failed assertion: `verify_comment_on_mssql` §5 asserts `comment IS NULL` in `duckdb_tables()` for a table whose `MS_Description` the same section has just read back off the server. ⚠ The warehouse refusal is **UNMEASURED on those engines** — there is no live warehouse here, so `ServerProfile.SupportsExtendedProperties` is conservative by choice; being wrong that way refuses a statement that might have worked, where the other way would issue one that fails and, inside an explicit transaction on those engines, abort it |
