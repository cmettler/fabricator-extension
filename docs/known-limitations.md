...
| 1.21 | **`COMMENT ON` reads back, but a comment changed OUT OF BAND is not picked up until the catalog cache is refreshed.** Also **refused on Fabric Warehouse / the Fabric Lakehouse SQL endpoint / Synapse dedicated** (no extended properties), on a provider-declared VIEW, and on the DAX provider | ⚠ **THIS ROW USED TO SAY `COMMENT ON` WAS WRITE-ONLY AND THAT NOTHING READ A COMMENT BACK.** The read-back was built 2026-09-08 (`duckdb_tables().comment` / `duckdb_columns().comment`), so that half is GONE and the row is rewritten rather than deleted — the characterization test which pinned `comment IS NULL` is likewise REPLACED, in `verify_comment_on_mssql` §5, because falsifying it is the change announcing itself. What remains is staleness: a comment rides `table_info`, i.e. entry materialization, so it is as fresh as the cached ENTRY — `fabricator_refresh_cache()` or a re-ATTACH picks up a foreign change, and a write through the catalog is visible at once (both cache layers are invalidated, and each is mutation-tested: dropping the host's entry eviction kills both suites at their first read-back row, dropping the provider's database-wide comment cache kills the SQL Server one only, with Delta green at 53 as the control). ⚠ A Delta TIME-TRAVEL (`AT`) entry reports no TABLE comment — its schema comes from `GetSchemaAt`, which does not carry `description`, and reporting the CURRENT comment on a historical entry would be a wrong answer rather than a missing one. **MEASURED UNOBSERVABLE from SQL**, which is why it is recorded here as an internal asymmetry rather than as something to work around: AT entries are cached separately and are NOT enumerated (`duckdb_tables()` shows exactly ONE row for a table referenced with an AT clause), so nothing reads an AT entry's comment. It becomes real only if AT entries ever gain a metadata surface. ⚠ The warehouse refusal is still **UNMEASURED** on those engines |
...
| 1.22 | **`information_schema.columns.column_default` reports a LITERAL default only.** A SQL Server column whose default is an expression — `getdate()`, `newid()`, `CONVERT(...)`, `(1)+(2)` — reads back as NULL, i.e. indistinguishable from a column with no default at all. Delta reports NO defaults at all | ⚠ **A DECISION, not a shortfall, and the safe half of a choice with no safe middle.** A default on a DuckDB `ColumnDefinition` is not display metadata: `bind_insert.cpp`'s `ExpandDefaultExpression` SUBSTITUTES it for a column an INSERT omits. For a literal that is the value the server would have applied anyway; for `getdate()` it would be the CLIENT's clock instead of the server's — a silently different row — and for a function DuckDB does not have, a bind error on a statement that works today. There is no "report it but stop the binder using it": display (`duckdb_columns`, `pragma_table_info`, the CREATE-SQL renderer) and semantics (`bind_insert`, `bind_create_table`, the Appender) read the SAME field. So the host keeps only what DuckDB parses as a single CONSTANT — the line DuckDB draws for itself in `transform_alter_table.cpp`. ⚠ Withholding COSTS NOTHING at INSERT time: with nothing reported the server applies its own default, which is measured and gated (`verify_alter_default` §5 asserts a withheld `getdate()` column still gets the server's clock). Delta reports none because engineered-wood implements no part of the column-default writer feature — the same reason the write side refuses there. Two mutants, each killed at its own row: honouring a non-constant dies on the withheld row, dropping the cache invalidation dies on the out-of-band row beside it |
| ~~1.23~~ | **FIXED 2026-09-10 by making it a SETTING, because it is not solveable as a policy — user-decided.** The defect: an explicit `NULL` written into a SQL Server column that HAS a default was replaced by that default, so a caller who deliberately wrote NULL got a different value, silently. **`mssql_keep_nulls` now DEFAULTS TRUE** (a NULL is written as NULL) with a `keep_nulls` ATTACH option beside it. ⚠ What REMAINS, and it is the reason no policy can be right: `VALUES (…, DEFAULT)` on a column whose default is an **EXPRESSION** now stores NULL instead of the server's value | **MEASURED with a control that isolates the write path**: through the catalog, `VALUES (4, NULL, NULL)` stored `10` and a `getdate()` timestamp while the byte-identical statement as plain T-SQL through `fabricator_exec` stored `NULL, NULL`; and a COPY of a CSV with a BLANK field stored `99`. Cause: INSERT/CTAS/COPY stream through `SqlBulkCopy`, which without `KeepNulls` replaces a source NULL with the destination default. **⚠⚠ WHY A SETTING RATHER THAN A FIX:** DuckDB's binder collapses an explicit NULL and an explicit `DEFAULT` keyword into the SAME typed NULL before our sink sees the chunk (`ExpandDefaultExpression`), so nothing at this layer can tell them apart — TRUE serves the explicit NULL, FALSE serves `DEFAULT`-on-an-expression-default, and there is no third option (by the time `PlanInsert` runs the substitution has already happened). ⚠ **An OMITTED column is unaffected by either value** — DuckDB's `column_index_map` leaves it out of the batch entirely, so it never becomes a NULL to replace and the server applies its own default; measured on both values, including for a withheld default. ⚠ A `DEFAULT` on a LITERAL default works under either value, because the column-default read-back (1.22) reports it and DuckDB substitutes it itself — so 1.22 genuinely shrinks the affected set. Gate: `verify_keep_nulls` (68), three mutants each killed at its own section. ⚠ **An earlier note here claimed `KeepNulls` "would break the omitted-column case" — that was WRONG** (inferred from `ExpandDefaultExpression` without checking which caller reaches it) and is what made this look unfixable for a day |
| 1.24 | **An object the provider cannot DESCRIBE is omitted from the catalog listing** — most commonly an **INVALID view** on SQL Server (one whose underlying table or column has been dropped or renamed), but equally a table the principal cannot read or one whose types do not map. It is skipped with a WARNING in `duckdb_logs`; a DIRECT reference still fails, loudly | **The alternative is what this replaced, and it was worse in three ways (user-reported against a production database, fixed 2026-09-11).** Enumeration used to FAIL: `duckdb_tables()` segfaulted — MEASURED on 3 runs in 4 over a catalog of 20 invalid views — because SQL Server's error **208 names the object that is MISSING, which for an invalid view is the REFERENT, not the view**; the host read that as "this view is absent" and ERASED it from the catalog *while the enumeration was iterating the map it erases from*. Two use-after-frees (the freed node's key was also still being used as the erase argument) plus a wrong answer about a live object. ⚠ **A listing is what dbt and every BI tool run BEFORE they do anything**, so a single broken view made the whole catalog unusable. Now: 208 is classified by ASKING the server whether the object itself exists (`OBJECT_ID`, one round trip on the failure path only), enumeration tolerates a per-table failure, and a direct reference reports SQL Server's own message naming the real culprit instead of "does not exist". ⚠ An undescribable object therefore cannot appear in `duckdb_tables()` / `information_schema.tables` at all — a catalog entry needs columns — which is the residual limitation; the WARNING is what keeps it from being silent. Gate: `verify_invalid_objects` (36) |
| 1.25 | **A bare `LIMIT` above a STREAMING TABLE-IN-OUT function fails with `Invalid Error: resource deadlock would occur`** — `SELECT … FROM my_inout(…) LIMIT 1`. Generic to the exchange operator, so it affects `fluid_query_inout`, the first-party global in-outs, and every provider-declared `_each`. ⚠ `ORDER BY … LIMIT` is FINE | **PRE-EXISTING and MEASURED as such**: reproduced on `fluid_query_inout` AND on `fabricator_inout_va`, and on a binary built from HEAD with the 2026-09-12 projection-pushdown change absent — which is what attributes it away from that work. Mechanism: the operator returns `HAVE_MORE_OUTPUT` while holding `FabricatorExchangeGlobalState::gate`; a bare `LIMIT` satisfies itself and the pipeline stops pulling, so the gate is never released, and `OperatorFinalize` → `ExchangeHolder::Finish` → `FinishEof` then takes `lock_guard<mutex>` on that same gate **from the same thread**. ⚠ `ORDER BY` is a pipeline BREAKER, so it drains the in-out fully before the limit applies — which is why this went unnoticed, since that is the spelling the suites use. ⚠⚠ MSVC's `std::mutex` DETECTS the same-thread re-lock and throws; **re-locking a non-recursive mutex is UNDEFINED BEHAVIOUR on glibc and typically HANGS**, so the Linux symptom is likely worse than the Windows one — the same asymmetry recorded for `entry_lock_`. **UNFIXED and UNGATED**: a gate for it would need the fix, and writing one now would have parked a failing assertion in the suite. Full record: [fluid-templating.md](fluid-templating.md) §37.5 |
| ~~1.20~~ | **WITHDRAWN 2026-08-22 — there was no defect. The raw loadable loads fine in a stock DuckDB 1.5.5 wheel; what crashed was MY TEST VENV.** Kept as a row rather than deleted because the methodology lesson is worth more than the row was: **before recording a defect seen in a local environment, verify the environment is the one users have.** | Bounded by loading a CI-BUILT artifact (the v0.0.11 windows Standalone asset) on this machine — it segfaulted too, which excluded our build tree — and then in a FRESH venv holding only the official `duckdb==1.5.5` wheel, where **both** the CI artifact and our local loadable load and report `0.0.11`. The culprit: the probe venv had duckdb pip-installed **from a local source tree** (`direct_url.json` = `file:///.../scratchpad/pyduck`, a `project(duckdb_python)` checkout), so its engine is a different build of the same version — 41,451,008 bytes against the official wheel's 37,374,464, and `Requires-Python >=3.11` against `>=3.10`. A version MATCH with an ABI MISMATCH is exactly a crash at load. ⚠ Note the sequence of my own errors: probe -> record a limitation -> narrow it twice -> discover it was never real. The CI evidence that made me downgrade it was right, and still not the same thing as bounding it. ⚠ Consequence worth keeping: dbt-duckdb and the notebook flow are UNAFFECTED, and `pyarrow` was tested and exonerated |
