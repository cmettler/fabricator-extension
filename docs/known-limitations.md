...
| 1.23 | **⚠ An EXPLICIT `NULL` written into a SQL Server column that HAS a default is replaced by that default.** `INSERT INTO t (k, v) VALUES (1, NULL)` stores the default, not NULL — so a caller who deliberately writes NULL gets a different value, silently | **MEASURED 2026-09-09 with a control that isolates the write path**: through the catalog, `VALUES (4, NULL, NULL)` stored `10` and a `getdate()` timestamp; the byte-identical statement as plain T-SQL through `fabricator_exec` stored `NULL, NULL`. Same server, same table, same statement — only the path differs. **Cause:** INSERT/CTAS/COPY go through `SqlBulkCopy`, and `SqlBulkCopyOptions.KeepNulls` is never set; without it SqlClient documents source NULLs as being replaced by the destination column's default. ⚠ **NOT fixed here, because the two halves are coupled and the fix is not one flag.** Setting `KeepNulls` would make an INSERT that OMITS a defaulted column store NULL instead of the default — today that works only BECAUSE the replacement is on. What makes both correct is reporting the default (1.22) so DuckDB substitutes it explicitly and "omitted" stops being indistinguishable from "explicit NULL" at our layer — which now holds for LITERAL defaults and not for the rest. Its own change, with its own gate |
