...
| 1.20 | **The RAW loadable SEGFAULTS at `LOAD` inside a stock DuckDB 1.5.5 python wheel** (exit 139, no message). That breaks the two documented flows that load it that way: **dbt-duckdb** ([dbt-incremental.md](dbt-incremental.md), [dbt-hooks.md](dbt-hooks.md)) and the **Fabric notebook** path. The statically-linked `unittest`/`duckdb.exe` are unaffected, which is why both CI tiers are green through it | Measured 2026-08-22 in a venv whose `duckdb` is exactly 1.5.5 (the footer reads `CPP | 0.0.11 | v1.5.5 | windows_amd64 | 4`, so the version matches). **Not caused by the parallel-write work**: `git checkout <pre-change> -- src/` + rebuild reproduces it identically. **Not the CLR boot**: pointing `FABRICATOR_MANAGED_DIR` at a nonexistent directory STILL segfaults, where that path is documented to fail cleanly with *"failed to load hostfxr"*, and no `FABRICATOR_LOG_FILE` line is ever written — so it is inside the extension load/registration, before any managed code runs. **The version-rejection path is intact**: the same artifact against duckdb 1.5.2 gives the clean *"built specifically for DuckDB version 'v1.5.5'"* error and exit 0, so the crash needs a version MATCH, i.e. a load that actually initializes. ⚠ **NARROWED the same day, and it downgrades this row: the SHIPPED artifact is very likely NOT affected.** The distribution tier passed on **2026-08-20 at `0cff38c` (v0.0.11)** on all four platform x SKU jobs, and its 12-check smoke does a cold `LOAD` against a STOCK DuckDB wheel *and* chain-loads the extracted raw `fabricator_core` into it — i.e. exactly the operation that crashes here, working in CI on three platforms against code one version bump below this. So the fault is most likely LOCAL to this build tree (its vcpkg openssl/curl/yyjson, its configure, this machine) rather than in the product. ⚠ Still real and still worth bounding, because it blocks the dbt-duckdb harness, the notebook validation flow, and any measurement needing two connections in one process — but do NOT treat it as a release blocker. Next step is a discriminating one: compare a CI-built core against the local one in the same wheel |
