...
The compose setup replaces the old ad-hoc `mssql-arrownet` container and adds MinIO so the Delta
...
docker stop mssql-arrownet; docker rm mssql-arrownet

# 1. generate the MinIO TLS cert (gitignored; SANs: minio, localhost, 127.0.0.1)
pwsh docker/certs/generate-certs.ps1

# 2. start everything (SQL Server 1433, MinIO S3 9000 / console 9001, bucket `arrownet`)
...
| MinIO | `miniouser` / `miniosecret123` (alphanumeric — SQL's S3 credential requires it), bucket `arrownet` |
...
```bash
export ARROWNET_S3_ENDPOINT=localhost:9000   # gates test/verify_delta_catalog_s3.test
export ARROWNET_S3_SQL_ENDPOINT=minio:9000   # gates test/verify_mssql_s3_polybase.test (with MSSQL_TESTDB_DSN)
```
