...
#
# One runner for both so the assertions below â the part with the actual value â exist once. Used by
...
#
# ASSERTIONS THIS MAKES, beyond a zero exit status â none of which `unittest` gives you for free:
...
case "$TIER" in
    hermetic)
        ...
        # 4206 since 2026-07-29: verify_delta_clustered_optimize gained 9 (Â§8 pins that a clustering-declared
        ...
        # SECOND time on the codec engine (+1065 â write 31, transactions 943, update 63, delete 28, the
        ...
        # compatibility â native_read/native_write are INDEPENDENT options, so besides the two symmetric
        ...
        # 5471 since 2026-07-30: verify_delta_row_level_concurrency gained 12 (Â§10 pins that a row-level
        # DELETE under `serializable` still lands across a concurrent COMPACTION â it ABORTED before the
        # engineered-wood bump, and nothing covered it on either side).
        # 61 runs / 5521 since 2026-07-30: verify_macros_catalog (50) pins CATALOG-BOUND provider macros â
        ...
        # CALLER's search path + TimeZone, so `USE s; SELECT â¦ FROM fabricator_host_query('â¦ FROM t')` resolves
        ...
        # computation. Suite count unchanged â the suite already existed.
        # 62 runs / 5558 since 2026-07-30: verify_delta_catalog_functions (21) pins CATALOG-BOUND CUSTOM
        # FUNCTIONS on the Delta catalog â until then all seven of its function ABI members threw and the
        ...
        # custom table function â both `:=` and `=>` spellings, that the argument really crosses the ABI (an
        ...
        # 5573 since 2026-07-31: verify_global_functions gained 9 (63 -> 72) â the demo global fabricator_seq
        ...
        # 63 suites / 5607 since 2026-07-31: NEW verify_delta_last_checkpoint (34) â an empty/corrupt/
        ...
        # Pinned hermetically by writing the corrupt states directly â a live race only sometimes collides,
        ...
        # automatic create-time delta.isolationLevel stamp. The added sections pin the DEFAULT itself â every
        # other isolation assertion in the tree now states its level explicitly, so without this a regression
        # in the default would fail nothing â plus that neither level auto-stamps and that an explicit
        # WITH ("delta.isolationLevel"=...) still does.
        # 5639 since 2026-08-01: verify_delta_row_level_concurrency 82 -> 93 (Â§11). The EW bump made
        ...
        # was left unset â Â§11 is the section that fails. It drives a non-pushable DELETE (so the scan
        ...
        # 5654 since 2026-08-02: verify_delta_txn_version 51 -> 65, Â§9 â a REFUSED flush now takes back the
        ...
        # grouping's threshold is 64 MiB of Arrow data and nothing else here comes near it â without this
        # suite the grouped path has NO coverage at all, at either tier.
        : "${MIN_SUITES:=66}"
        # 5656 since 2026-08-02: verify_delta_catalog_transactions 943 -> 944 â ROLLBACK now RECLAIMS the
        ...
        # refusal â the shared C++ CreateTable never checked ERROR_ON_CONFLICT, so a plain CTAS wrote no rows
        # and kept the OLD data (exit 0, no error) and a plain CREATE silently ignored its DECLARED SCHEMA.
        # â  +24, not +12: verify_delta_catalog_write is one of the four DOUBLED suites, so each assertion
        # counts once per engine â a floor of 5668 from the standalone delta would have tolerated a
        # 12-assertion regression, the same trap the s3 note records on the service tier.
        # â  5686 - 5656 = 30 of this gap PREDATES the change: the floor was not raised for the unified
        ...
        # verify_merge_into (130) joins the DOUBLED list below â it is composed of exactly the update/delete/
        ...
        # vectors DISABLED and is REFUSED inside a transaction (the buffered path requires them â so reaching
        ...
        # MULTI-ACTION merge became FORCED-BUFFERED. That was not a feature â it fixed SILENT DATA DESTRUCTION.
        ...
        # non-DV table, two conditional deletes left the wrong survivors and DESTROYED a row. Â§11 is that
        # regression gate (refusal + table intact + a single-action positive control), Â§11b the same shape on a
        # DV table asserting the answer AND the fusion. â  Both tiers were GREEN THROUGH the bug, because every
        # earlier test put the affected rows in SEPARATE FILES where a rewrite renumbers nothing â so a floor
        ...
        # sides of each boundary, which is the point â a guard wider than its hazard fails as a lost capability,
        ...
        # literal per deleted row â MEASURED ~0.4ms and ~1.2KB PER DELETED ROW (a 200k-row table with 199k
        ...
        # section (each file binds its own single-use stream â a regression to one shared stream is invisible
        ...
        # 6367 since 2026-08-06: verify_delta_update_grouped (72) â a large UPDATE now writes its post-images
        ...
        # is the BOXED SET-value dictionary built before any provider work â see DeltaReader.UpdateGroupBytes).
        # The suite updates 6000 rows on purpose: the read-back yields ~2048 rows per batch, so it spans three
        # groups. Mutation-tested â not clearing the per-group id list dies at the FIRST grouped UPDATE
        ...
        # 6389 since 2026-08-06: verify_delta_catalog_update 73 -> 84 (x2 legs), §"a rowid matched more than
        # once". `UPDATE … FROM src` whose join matches one target row several times sends that rowid down
        ...
        # the compaction: the no-duplication assertion still PASSED and the membership one caught it — one
    ...
    service)
        ...
        # (+161, the same count as its native leg). 42 suites, 43 runs â the floor is on RUNS.
        # 44 RUNS / 1413 since 2026-07-31: verify_session_tag (+25) â fabricator_session_tag, which needs a
        # real server (it pins a provider connection and reads the session's own monitoring ids).
        : "${MIN_SUITES:=45}"
        # 1424 since 2026-08-01: verify_exec_invalidate_cache 10 -> 21, for the OUT-OF-BAND DROP path â the
        ...
        # "does not exist" WITHOUT ever fetching columns â the section then passes with the provider's
        # absence detection disabled, which is exactly what mutation-testing caught it doing.
        # 1444 since 2026-08-02: verify_delta_catalog_s3 161 -> 171, Â§11 â the attach-time warning for an
        # s3:// root opened READ_WRITE with no NAMED secret. MEASURED first: that shape loses 40 of 48
        # concurrent commits SILENTLY (Â§8.3). Two mutants, killed in opposite directions by ONE assertion â
        ...
        # â  +20, not +10: verify_delta_catalog_s3 is THE DOUBLED SUITE of this tier (see DOUBLED below), so
        ...
        # 1465 since 2026-08-04: verify_ctas_text_type 8 -> 15 â the SQL Server half of the CREATE-conflict
        ...
        # â  1458 - 1444 = 14 of this gap PREDATES the change (the unified parameter protocol pass raised the
        # actual and not the floor). Closed here for the same reason as the hermetic one.
        # â  The floor is the MEASURED tier total (1465), not 1458 + the standalone delta. Both numbers in the
        ...
        # column that ReferenceKeyColumns destructures â a Delta rowid is always a single virtual BIGINT), and
...
if [ "$TIER" = 'hermetic' ]; then
    ...
    # would still be green, and the coverage would be silently gone â the exact failure mode this
...
suites=$("$SELECT_CMD")
if [ -z "$suites" ]; then
    echo "ERROR: no $TIER suites selected â $SELECT_CMD is broken." >&2
    exit 1
fi

# THE DOUBLED LEG. Since 2026-07-29 the PROVIDER NAME picks a default engine â 'delta' means DuckDB
...
#
#   hermetic â write / transaction / update / delete semantics must come out IDENTICAL either way.
#   service  â the S3 suite, because object storage is where the two engines' file handling has
...
while IFS="$(printf '\t')" read -r suite provider; do
    ...
    # statement here takes the single-group path and the grouped one is never entered. This suite â and
    # ONLY this suite â runs with the threshold at a single byte, making each read-back batch its own
    ...
    if [ "$status" -ne 0 ]; then
        ...
        reason='the filter matched no test case (exits zero â would be a silent no-op)'
...
# tree and â as happened on 2026-07-29 â gets its droppings COMMITTED by the next `git add`. That leak
...
if [ "$have_git" -eq 1 ]; then
    ...
    if [ "$fixtures_after" != "$fixtures_before" ]; then
        echo "ERROR: this run modified test/fixtures â those are committed, read-only inputs." >&2
...
if [ "$expected" -lt "$MIN_SUITES" ]; then
    ...
    echo "       require-env and silently left the $TIER set â or a doubled leg stopped being added." >&2
...
if [ "$assertions" -lt "$MIN_ASSERTIONS" ]; then
    echo "ERROR: $assertions assertions ran, floor is $MIN_ASSERTIONS â coverage went backwards." >&2
    exit 1
fi
