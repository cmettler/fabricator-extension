...
| [consumption-monitoring.md](docs/consumption-monitoring.md) | **analysis only, nothing built** (2026-07-31) — CU/consumption attribution for a dbt run. THREE tagging vectors CONFIRMED live (`OPTION (LABEL)` on all 5 statement shapes incl. CTAS; `Application Name`→`program_name`; `SET CONTEXT_INFO`→`exec_sessions_history.context_info`, while `sp_set_session_context` is monitoring-INVISIBLE). Records a live `application_name` DEFECT, the finding that consecutive extension calls do NOT share a session, and a REPORTED-not-confirmed Aug-2026 Warehouse metering change that would undercut per-model costing |
