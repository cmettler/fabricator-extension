...
- Suggested order: (1) **C# multi-backend registry — DONE** (`BackendRegistry` is provider-keyed:
  ...
  **+ (4f) custom table functions DONE** (`IArrowTableFunction`) — both reuse the catalog scalar/TVF path,
  C#-only, no ABI; chosen over load-time global (deferred). **(4g) table-in-out — DESIGNED, next to build**
  (full plan: [docs/custom-functions-design.md](docs/custom-functions-design.md) §11.1): a discovered TVF
  gains a `{LogicalType::TABLE}` overload that applies it **once per input row** via SQL-Server `CROSS APPLY`
  (T-SQL generated by C#, run on SQL Server — NOT a DuckDB lateral join), over the §11 coordinated bounded
  channel (one session per call; parallel `UNION ALL` input fed thread-safely). **Key (verified): `in_out_
  function_final` fires once PER parallel branch (twice for a 2-way UNION ALL) → unusable as the "all input
  done" signal → the injected `OperatorFinalize` operator (`LogicalExtensionOperator` + `OptimizerExtension`)
  is REQUIRED even for read-only** (single reliable finalize; NOT just for write/commit — that earlier note
  was wrong). TVFs are read-only (no commit); per-row **stored procs** (rollback-on-failure) + a **custom C#
  `IArrowTableInOutFunction`** (in-out analog of 4e/4f) come after. ABI: `inout_open`/`push`/`finish`/`abort`.
...
**Not yet / out of scope:** **table-in-out** functions (4g — *designed, next to build*; see §11.1 + the
sequencing note above) and **load-time global** functions
