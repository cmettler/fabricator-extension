//===----------------------------------------------------------------------===//
//                         fabricator — filesystem reverse-callback SPIKE (impl)
//===----------------------------------------------------------------------===//
...
int32_t HostFsSize(FabricatorHandle file, int64_t *out_size, char **err) {
	try {
...
int32_t HostFsRead(FabricatorHandle file, void *buffer, int64_t nr_bytes, int64_t location, char **err) {
	try {
...
void HostFsClose(FabricatorHandle file) {
...
int32_t HostFsOpenWrite(FabricatorHandle opener, const char *path, int32_t exclusive, FabricatorHandle *out_file,
	...
	try {
		...
		*out_file = reinterpret_cast<FabricatorHandle>(handle.release()); // closed via HostFsCloseWrite
		return FABRICATOR_OK;
	} catch (std::exception &e) {
...
int32_t HostFsWrite(FabricatorHandle file, const void *buffer, int64_t nr_bytes, char **err) {
	try {
...
int32_t HostFsCloseWrite(FabricatorHandle file, char **err) {
	auto *h = reinterpret_cast<FileHandle *>(file);
	try {
...
// -----------------------------------------------------------------------------
// fabricator_fs_spike(path) table function — a ClientContext-bearing trigger that asks the managed side to do
...
void FsSpikeFunc(ClientContext &context, TableFunctionInput &data, DataChunk &output) {
	...
	string result = fabricator::FsSpike(reinterpret_cast<FabricatorHandle>(&context), bind_data.path);
	output.SetCardinality(1);
	output.SetValue(0, 0, Value(result));
}

// -----------------------------------------------------------------------------
// fabricator_fs_write_probe(base_path) — capability probe for the WRITE side of DuckDB's FileSystem (the
...
void FsWriteProbeFunc(ClientContext &context, TableFunctionInput &data, DataChunk &output) {
	...
	const string dir = bind_data.base + "/fabricator_write_probe";
	...
	const string content = "hello-fabricator";
...
void RegisterFsSpike(ExtensionLoader &loader) {
	...
	TableFunction fn("fabricator_fs_spike", {LogicalType::VARCHAR}, FsSpikeFunc, FsSpikeBind, FsSpikeInit);
	loader.RegisterFunction(fn);
	TableFunction probe("fabricator_fs_write_probe", {LogicalType::VARCHAR}, FsWriteProbeFunc, FsWriteProbeBind,
	                    FsWriteProbeInit);
