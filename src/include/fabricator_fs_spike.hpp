//===----------------------------------------------------------------------===//
//                         fabricator — filesystem reverse-callback SPIKE
//
...
// DuckDB's FileSystem) and registers an `fabricator_fs_spike(path)` table function
...
//! `fabricator_fs_spike(path VARCHAR)` table function. Call early in extension load.
