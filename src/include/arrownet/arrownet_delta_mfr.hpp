//===----------------------------------------------------------------------===//
// arrownet/arrownet_delta_mfr.hpp
//
// Delta native-read via DuckDB's MultiFileReader (docs/multifile-delta.md Phase A). Registers
// `arrownet_delta_mfr_scan(path)` — a clone of parquet_scan whose MultiFileReader gets the EXACT active data
...
// Register the arrownet_delta_mfr_scan table function (best-effort; requires the parquet extension, which is
