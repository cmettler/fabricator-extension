...
// An mssql_net scan Get found directly under a node, through at most one Projection.
...
bool ResolveOrderColumn(const Expression &expr, LogicalGet &get, LogicalProjection *proj,
                        arrownet::ArrowStreamBindData &bind_data, string &name, bool &nullable, bool &is_string) {
...
// LIMIT n (constant, no/zero offset) over an mssql_net scan -> SELECT TOP (n).
void TryPushLimit(LogicalOperator &op) {
	...
	match.get->bind_data->Cast<arrownet::ArrowStreamBindData>().top_n =
	    static_cast<int64_t>(limit.limit_val.GetConstantValue());
}

// TopN (ORDER BY + LIMIT, fused by the built-in TOP_N optimizer) over an mssql_net
...
void TryPushTopN(ClientContext &context, LogicalOperator &op) {
	...
	auto &bind_data = match.get->bind_data->Cast<arrownet::ArrowStreamBindData>();
