//===----------------------------------------------------------------------===//
//                         fabricator — table catalog entry (impl)
//===----------------------------------------------------------------------===//
...
// Shared with the table-function scan (fabricator_schema_entry.cpp); declared in
// fabricator_table_entry.hpp.
void FabricatorComplexFilterPushdown(ClientContext &context, LogicalGet &get, FunctionData *bind_data_p,
                                   vector<unique_ptr<Expression>> &filters) {
	auto &bind_data = bind_data_p->Cast<fabricator::ArrowStreamBindData>();

	// Diagnostic (duckdb_logs, type 'Fabricator.Pushdown'): DuckDB may call this callback MORE THAN ONCE per
	...
	auto log_result = [&](idx_t serialized) {
		...
		if (!logger.ShouldLog("Fabricator.Pushdown", LogLevel::LOG_INFO)) {
			return;
		}
		string exprs;
		for (auto &f : filters) {
		...
		logger.WriteLog("Fabricator.Pushdown", LogLevel::LOG_INFO,
...
static vector<column_t> FabricatorScanRowIdColumns(ClientContext &context, optional_ptr<FunctionData> bind_data_p) {
...
TableFunction FabricatorTableEntry::BuildScanFunction(ClientContext &context, unique_ptr<FunctionData> &bind_data,
	...
	vector<string> names;
	fabricator::PopulateReturnSchema(context, *data, return_types, names);
	...
	function.pushdown_complex_filter = FabricatorComplexFilterPushdown;
	...
	if (ParentCatalog().Cast<FabricatorCatalog>().ExactFilterPushdown()) {
		...
		if (HasVirtualRowId() && rowid_type_.id() == LogicalTypeId::BIGINT) {
			...
			function.get_row_id_columns = FabricatorScanRowIdColumns;
		}
	}
	function.cardinality = FabricatorScanCardinality;
	function.statistics = FabricatorScanStatistics;
	function.get_bind_info = fabricator::ArrowStreamGetBindInfo;
	return function;
}

unique_ptr<BaseStatistics> FabricatorTableEntry::GetStatistics(ClientContext &context, column_t column_id) {
	return nullptr;
}
...
virtual_column_map_t FabricatorTableEntry::GetVirtualColumns() const {
	...
	for (idx_t i = 0; i < provider_virtual_columns_.size(); i++) {
		result.insert(make_pair(fabricator::ProviderVirtualBase() + i,
...
vector<column_t> FabricatorTableEntry::GetRowIdColumns() const {
	vector<column_t> result;
	if (HasRowId()) {
