...
static unique_ptr<BaseSecret> CreateProviderSecret(ClientContext &context, CreateSecretInput &input) {
	...
	if (it == g_secret_types.end()) {
		throw InvalidInputException("fabricator: unknown secret type '%s'", input.type);
	}
...
void RegisterProviderSecrets(ExtensionLoader &loader) {
	try {
		...
		fabricator::ListSecretFields(stream);
...
bool IsKnownSecret(ClientContext &context, const string &secret_name) {
	...
	// ANY existing secret (our own mssql type OR a foreign one reused for auth, e.g. azure). A foreign
...
string BuildConnectionStringFromSecret(ClientContext &context, const string &secret_name,
	...
	if (!entry || !entry->secret) {
		throw BinderException("fabricator: secret '%s' not found. Create it with: CREATE SECRET %s "
		                      "(TYPE mssql, host '...', database '...', user '...', password '...')",
	...
	// validates them (build_connection_string), interpreting them per `secret_type` (our own mssql secret
