...
class FabricatorOneLakeFileHandle : public FileHandle {
public:
	FabricatorOneLakeFileHandle(FileSystem &fs, const string &path, FileOpenFlags flags, FabricatorHandle handle,
	...
	~FabricatorOneLakeFileHandle() override {
		Close();
	}
	void Close() override {
		if (managed_handle) {
	...
	FabricatorHandle managed_handle;
	int64_t file_size;
	idx_t position;
	bool write_mode;
...
class FabricatorOneLakeFileSystem : public FileSystem {
public:
...
private:
	unique_ptr<FileHandle> OpenInternal(const string &path, FileOpenFlags flags, optional_ptr<FileOpener> opener,
		...
		if (flags.OpenForWriting()) {
			...
			return make_uniq<FabricatorOneLakeFileHandle>(*this, path, flags, handle, 0, /*is_write=*/true);
		}
		int64_t size = 0;
		...
		return make_uniq<FabricatorOneLakeFileHandle>(*this, path, flags, handle, size, /*is_write=*/false,
...
public:
	...
	void Read(FileHandle &handle, void *buffer, int64_t nr_bytes, idx_t location) override {
		auto &h = handle.Cast<FabricatorOneLakeFileHandle>();
	...
	int64_t GetFileSize(FileHandle &handle) override {
		return handle.Cast<FabricatorOneLakeFileHandle>().file_size;
	}
	...
	string GetVersionTag(FileHandle &handle) override {
		return handle.Cast<FabricatorOneLakeFileHandle>().etag;
	}
	timestamp_t GetLastModifiedTime(FileHandle &handle) override {
		return handle.Cast<FabricatorOneLakeFileHandle>().last_modified;
	}

	void Seek(FileHandle &handle, idx_t location) override {
		handle.Cast<FabricatorOneLakeFileHandle>().position = location;
	}
	idx_t SeekPosition(FileHandle &handle) override {
		return handle.Cast<FabricatorOneLakeFileHandle>().position;
	}
	void Reset(FileHandle &handle) override {
		handle.Cast<FabricatorOneLakeFileHandle>().position = 0;
	}
	...
	std::string GetName() const override {
		return "FabricatorOneLakeFileSystem";
	}
};
...
void RegisterOneLakeFileSystem(DatabaseInstance &db) {
	...
	fs.RegisterSubSystem(make_uniq<FabricatorOneLakeFileSystem>());
}
