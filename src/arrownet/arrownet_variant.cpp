...
constexpr const char *kVariantExtensionName = "arrownet.variant";
...
// Import schema: a field tagged arrownet.variant resolves to DuckDB VARIANT; the ArrowType carries the
...
unique_ptr<ArrowType> GetVariantType(ClientContext &context, const ArrowSchema &schema,
	...
	throw InvalidInputException(
	    "arrownet.variant expects a binary transport column (metadata||value), got format \"%s\"",
	    format.c_str());
}

} // namespace

void RegisterArrowNetVariantExtension(DatabaseInstance &db) {
