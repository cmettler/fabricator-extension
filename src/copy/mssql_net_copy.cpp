...
struct ArrowNetCopyGlobalState : public GlobalFunctionData {
	...
	~ArrowNetCopyGlobalState() override {
		...
		if (owned_catalog) {
			try {
				arrownet::SetActiveTxn(owned_catalog, txn_id);
				arrownet::RollbackTransaction(owned_catalog);
			} catch (...) {
			}
			try {
				arrownet::CloseCatalog(owned_catalog);
			} catch (...) {
			}
	...
	ArrowNetHandle owned_catalog = nullptr;
	int64_t txn_id = 0;
	mutex lock;
};
...
static void BindCommonCopyOptions(const case_insensitive_map_t<vector<Value>> &options,
                                  ArrowNetCopyBindData &bind_data, const char *fmt) {
...
static unique_ptr<FunctionData> CopyToBind(ClientContext &context, CopyFunctionBindInput &input,
	...
	BindCommonCopyOptions(input.info.options, *bind_data, "mssql_net");
...
static unique_ptr<FunctionData> DeltaCopyToBind(ClientContext &context, CopyFunctionBindInput &input,
	...
	// The legacy CREATE_TABLE/REPLACE/PARTITION_OVERWRITE flags (shared with FORMAT mssql_net) still work,
...
static unique_ptr<GlobalFunctionData> CopyToInitGlobal(ClientContext &context, FunctionData &bind_data_p,
	...
	if (bind_data.transient_delta) {
		...
		handle = arrownet::OpenCatalog(bind_data.catalog_name, "delta", bind_data.options_json);
		gstate->owned_catalog = handle;
		arrownet::SetActiveTxn(handle, gstate->txn_id);
	}
	...
	gstate->bulk_session = arrownet::BeginBulk(handle, bind_data.schema_name, bind_data.table_name,
...
static void CopyToFinalize(ClientContext &context, FunctionData &bind_data_p, GlobalFunctionData &gstate_p) {
	...
	if (bind_data.transient_delta) {
		...
		arrownet::SetActiveOpener(reinterpret_cast<ArrowNetHandle>(&context));
		arrownet::SetActiveTxn(handle, gstate.txn_id);
		arrownet::CommitTransaction(handle);
