...
public interface IBackendCatalog : IDisposable
{
    ...
    /// the constant NAMED arguments — a lateral function's positional parameters are its per-row input columns
    /// and carry no bind-time value; <paramref name="inputSchema"/> is those input columns.
    /// </summary>
    /// <remarks>
    /// A throwing default rather than a required member so no existing provider has to implement it: a
