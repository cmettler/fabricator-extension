...
namespace Fabricator.Bridge.Conversion;
...
public sealed class ColumnAppender
{
    ...
    public static ColumnAppender Create(IArrowType type)
    {
        switch (type.TypeId)
        {
            ...
            default:
                throw new NotSupportedException($"Fabricator: no column appender for Arrow type {type.TypeId} ({type.Name})");
        }
    }
    ...
    private static byte[] UuidBytes(object o) => o switch
    {
        ...
        _ => throw new NotSupportedException($"Fabricator: cannot convert {o.GetType().Name} to a UUID"),
    };
}
