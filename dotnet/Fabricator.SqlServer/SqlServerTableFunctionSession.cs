...
// ITableFunctionSession implementations for the Phase 5 table-function session model (tablefn_bind / tablefn_execute /
...
// (TableFunctionBindingAdapter — the ITableFunctionBinding wrapper used by stored procs, custom, and global table
...
internal sealed class TvfTableFunctionSession : ITableFunctionSession
{
    private readonly SqlServerTableValuedFunction _tvf;
    private readonly RecordBatch _args;

    public TvfTableFunctionSession(SqlServerTableValuedFunction tvf, RecordBatch args)
    {
        _tvf = tvf;
        _args = args;
    }
