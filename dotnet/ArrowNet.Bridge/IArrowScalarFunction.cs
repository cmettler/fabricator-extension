...
/// these (the SqlServer provider lists them in its <c>CustomFunctions</c> registry); they are
/// surfaced into every attached catalog's schema at discovery time alongside the provider's
/// discovered functions, so a custom function resolves as <c>db.SchemaName.Name(args)</c> and runs
/// through the very same scalar-function path as a discovered UDF — the catalog simply dispatches to
/// <see cref="Invoke"/> instead of generating SQL. No load-time/global registration, no extra ABI: it
/// reuses the existing <c>get_function_param_schema</c> / <c>get_function_return_schema</c> /
/// <c>execute_scalar</c>.
/// </summary>
public interface IArrowScalarFunction
{
