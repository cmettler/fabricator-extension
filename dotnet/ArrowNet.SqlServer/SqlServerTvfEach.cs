...
// Phase 6.2 streaming table-in-out binding (SqlServerTvfEach): applies a discovered TVF once per input row
// via SQL-Server CROSS APPLY, streamed on the gate-based exchange operator (IArrowInOutBinding). A nested
// partial of SqlServerCatalog so it keeps access to the catalog internals (BeginInOutScope /
// FunctionParameters / GetFunction*Schema / Quote / AddParameters) without widening them.
